...
import type { PublishProjectSchema } from "./schema";
 ...
 */
export const publishProject = async ({ projectId }: PublishProjectSchema) => {
  ...
  await db.transaction(async (tx) => {
    ...
    const layersFlat = rootProjectWithIds.pages
      .map((page) => page.layersToPages.map((layerToPage) => layerToPage.layer))
      .flat()
      .filter(notEmpty);
