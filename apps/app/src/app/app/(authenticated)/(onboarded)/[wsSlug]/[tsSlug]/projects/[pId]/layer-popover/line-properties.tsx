...
export function LineProperties({ form, datasetId, type }: LinePropertiesProps) {
  ...
  const availableLineColumns = getAvailableColumns("line");
  ...
  return (
    <>
      ...
      <DataColField
        availableColumns={availableLineColumns ?? []}
        datasetId={datasetId}
        form={form}
        label="Line"
        name="lineProperties.lineColumnId"
        type="line"
      />
      <DataColField
...
function DataColField({
  ...
  const { executeAsync } = useAction(createColumnAction, {
    onSuccess: ({ data, input }) => {
      ...
      if (input.type === "line") {
        form.setValue("lineProperties.lineColumnId", data.id);
      }
