  ...
  if (isMobile) {
    return (
      <>
        {open && depth === 0 && (
          <motion.div
            className="bg-background bg-opacity-98 group pointer-events-auto relative z-10 mt-[calc(100dvh-200px)] flex h-fit min-h-dvh !w-dvw flex-col bg-white pb-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] outline-none backdrop-blur-sm transition-[filter,width,padding-left] duration-[250] [--y-from:200px] [--y-to:0]"
            layoutScroll
            animate="open"
            initial="closed"
            exit="closed"
