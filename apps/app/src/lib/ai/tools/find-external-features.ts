...
export const findRawExternalFeatures = tool({
  ...
  inputSchema: z.object({
    query: z
      .string()
      .describe(
        "The name OR address of the place to search for. If you have both, ALWAYS search using the name. The tool returns results in order of relevance. Queries must be kept simple - no fuzzy searches! GOOD QUERY examples: 'Paris, France', 'Eiffel Tower', '123 Main St. San Francisco, CA', etc. BAD QUERY examples: 'Restaurants near Mile End Montreal', 'Maman Jeanne Montreal Ethiopian Restaurant address', 'La Banquise at 994 Rue Rachel E, Montreal'.",
      ),
    bounds: z
      .array(z.number())
      .optional()
